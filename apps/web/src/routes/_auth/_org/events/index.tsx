...
type SortKey = "title" | "date" | "status" | "visibility" | "registrationCount" | "category" | "price" | "location";
type EventSegment = "upcoming" | "archived";
...
const visibilities: EventVisibility[] = ["unpublished", "published", "archived"];
...
function Events() {
  ...
  const [segment, setSegment] = useState<EventSegment>("upcoming");
  ...
  const filteredEvents = useMemo(() => {
    ...
    return events
      .filter((event) => {
        const isUpcoming = event.status === "upcoming" && event.visibility !== "archived";
        const isArchived = event.status === "completed" || event.status === "cancelled" || event.visibility === "archived";
        const matchesSegment =
          segment === "upcoming" ? isUpcoming : isArchived;
        ...
        return matchesSegment && matchesSearch && matchesCategory && matchesStatus && matchesVisibility;
      })
  ...
  const handleCreate = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const assignments = resourceAssignments.filter((assignment) => assignment.resourceId && assignment.role.trim());
  ...
  const eventCounts = useMemo(() => {
    ...
    const archived = events.filter((e) => e.status === "completed" || e.status === "cancelled" || e.visibility === "archived").length;
              ...
              <PopoverContent className="w-[min(22rem,calc(100vw-2rem))] p-0" align="end">
                <div className="space-y-4 p-4">
                  ...
                  <div className="flex items-center justify-between border-t pt-3">
                    ...
                    <Button size="sm" className="h-8 px-3 text-xs" onClick={() => setFilterOpen(false)}>
                      Done
                    </Button>
                  </div>
                </div>
        ...
        {filteredEvents.length === 0 ? (
          <div className="rounded-xl border border-dashed bg-muted/30 p-12 text-center">
            ...
            <h2 className="mt-3 text-sm font-semibold tracking-tight">
              {segment === "upcoming" ? "No upcoming events" : "No archived events"}
            </h2>
            <p className="mx-auto mt-1 max-w-sm text-sm text-muted-foreground">
              {segment === "upcoming"
                ? "Create a new event to get started."
                : "Completed and cancelled events appear here."}
            </p>
          </div>
        ) : view === "list" ? (
          <EventsTable
        ...
        <TableHeader>
          <TableRow className="hover:bg-transparent">
            ...
            <SortableHead
              ...
              setSortKey={setSortKey}
            />
            <SortableHead label="Registrations" value="registrationCount" sortKey={sortKey} setSortKey={setSortKey} />
            <SortableHead
              label="Category"
              value="category"
              sortKey={sortKey}
            ...
            />
            <SortableHead label="Location" value="location" sortKey={sortKey} setSortKey={setSortKey} className="hidden xl:table-cell" />
            <SortableHead label="Price" value="price" sortKey={sortKey} setSortKey={setSortKey} className="hidden md:table-cell" />
...
function EventRow({
  ...
  const updateEvent = (input: Partial<Pick<EventDto, "status" | "visibility">>) => {
          ...
          <DropdownMenuContent align="end" className="w-40">
            ...
            {canManage && (
              <>
                ...
                <DropdownMenuItem
                  onClick={() => updateEvent({ visibility: event.visibility === "archived" ? "unpublished" : "archived" })}
                  disabled={patchMutation.isPending}
                  className="gap-2"
                >
                  {event.visibility === "archived" ? <ArchiveRestore className="size-3.5" /> : <Archive className="size-3.5" />}
                  {event.visibility === "archived" ? "Unarchive" : "Archive"}
                </DropdownMenuItem>
              </>
            )}
          </DropdownMenuContent>
  ...
  return (
    ...
    >
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        <VisibilityBadge visibility={event.visibility} />
        {event.category && <Badge variant="outline" className="h-5 px-1.5 text-[10px]">{event.category}</Badge>}
        <Badge variant="outline" className="h-5 px-1.5 text-[10px]">{event.price}</Badge>
...
function ResourceAssignmentEditor({
  ...
  const updateAssignment = (index: number, field: keyof ResourceAssignmentDraft, value: string | number) => {
  ...
  const selectedResourceIds = new Set(assignments.map((assignment) => assignment.resourceId).filter(Boolean));
    ...
    <div className="rounded-lg border p-4">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Assign instructors, locations, or materials after creation.</p>
        </div>
        <Button
          type="button"
          variant="outline"
      ...
      {resources.length === 0 ? (
        <p className="mt-3 text-sm text-muted-foreground">Create resources first to assign them here.</p>
      ) : assignments.length > 0 ? (
        <div className="mt-4 space-y-3">
          {assignments.map((assignment, index) => {
            const currentResource = resources.find((resource) => resource.id === assignment.resourceId);
            const availableResources = resources.filter(
              (resource) => resource.id === assignment.resourceId || !selectedResourceIds.has(resource.id),
            );
            return (
              <div key={index} className="grid gap-3 rounded-md border bg-muted/20 p-3 sm:grid-cols-[1fr_1fr_90px_auto] sm:items-end">
                <div className="space-y-2">
                  ...
                  <Select value={assignment.resourceId ?? ""} onValueChange={(value) => updateAssignment(index, "resourceId", value ?? "")}>
                    ...
                    <SelectContent alignItemWithTrigger={false}>
                      {currentResource && <SelectItem value={currentResource.id}>{currentResource.name} ({currentResource.type})</SelectItem>}
                      {availableResources.filter((resource) => resource.id !== currentResource?.id).map((resource) => (
                        <SelectItem key={resource.id} value={resource.id}>{resource.name} ({resource.type})</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                </div>
                <div className="space-y-2">
                  ...
                  <Input value={assignment.role} onChange={(event) => updateAssignment(index, "role", event.target.value)} placeholder="instructor, location" />
                </div>
                <div className="space-y-2">
                  <Label className="text-xs text-muted-foreground">Qty</Label>
                  <Input type="number" min="1" value={assignment.quantity} onChange={(event) => updateAssignment(index, "quantity", event.target.value)} />
                </div>
                <Button type="button" variant="ghost" size="sm" onClick={() => onChange(assignments.filter((_, i) => i !== index))}>
                  Remove
                </Button>
              </div>
            );
